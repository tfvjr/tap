...
// DetectProjectRoot to find the owning project. Processes that cannot be
// attributed are returned in the unattributed slice.
//
// The returned projects map is keyed by the absolute project root path.
func AttributeProcesses(processes []model.DevProcess) (projects map[string]*model.Project, unattributed []model.DevProcess) {
	...
	for i := range processes {
		...
		root, found := DetectProjectRoot(proc.ProjectPath)
		if !found {
			unattributed = append(unattributed, *proc)
			continue
		}
